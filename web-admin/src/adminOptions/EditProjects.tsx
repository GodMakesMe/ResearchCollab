...
const EditProjects: React.FC = () => {
  ...
  const handleEdit = (project: Project) => {
    // Trigger edit modal or redirect to a new page for editing
    // For example, we could open a modal with the project's details filled in
    console.log('Editing project:', project);
    // You can redirect or show a modal to edit the selected project
  };

  const handleSaveChanges = async (projectId: number, updatedData: Project) => {
    try {
      const token = localStorage.getItem('token');
      await axios.put(`${backend_url}/projects/${projectId}`, updatedData, {
        headers: {
          Authorization: `${token}`,
        },
      });
      alert('Project updated successfully!');
      // Refresh the list of projects after updating
      setPage(1);
    } catch (error) {
            ...
            <tbody>
              {projects.map((project, index) => (
                <tr key={project.id} style={{
                  ...
                  <td style={tdStyle}>
                    <button onClick={() => handleEdit(project)} style={{ ...buttonStyle, backgroundColor: 'orange' }}>
                      Edit
                    </button>
                  </td>
                </tr>
